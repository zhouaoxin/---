<template>
  <div style="width: 100%">
    <el-breadcrumb separator-class="el-icon-arrow-right">
      <el-breadcrumb-item :to="{ path: '/scenic' }">情感分析子系统</el-breadcrumb-item>
    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <el-button type="success" size="small" @click="addOrUpdateHandle(scope.row.scenicId)">查看 <i class="el-icon-info"></i></el-button>
          <el-popconfirm
              class="ml-5"
              confirm-button-text='确定'
              cancel-button-text='我再想想'
              ...
              icon-color="red"
              title="确定删除吗？不可逆"
              @confirm="deleteHandle(scope.row.scenicId)"
          >
...
<script>
import AddOrUpdate from './scenic-add-or-update'
...
export default {
  ...
  methods: {
    ...
    // 批量删除
    deleteBatch(){
      let ids = this.dataListSelections.map(v => v.scenicId)
      request.delete("/scenic/deleteBatch/",{data: ids}).then(res => {
        if (res){
    ...
    // 删除
    deleteHandle (id) {
      request.delete("/scenic/deleteById/"+id, id).then(res => {
        if (res){
